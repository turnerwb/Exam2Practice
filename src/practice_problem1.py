...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        if len(self.contents) > volume:
            self.contents = ""
        self.initvol = self.volume
        self.initcont = self.contents
    ...
    def append_string(self, additional_contents):
        ...
        #    and continue working on the problem.
        # --------------------------------------------------------------
        remaining = self.volume - len(self.contents)
        s = ""
        ...
        for k in range(remaining, len(additional_contents)):
    ...
    def double_then_shrink(self, new_volume):
        ...
        # --------------------------------------------------------------
        count = 0
        count += len(self.double())
        count += len(self.shrink(new_volume))
        return count
    def reset(self):
        """
        What comes in:
